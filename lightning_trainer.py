...
class MyCustomTrainer:
    def __init__(
        ...
        # Enzo: I added this line as well as gradient clipping before optimizer.step
    ...
    def fit(
        ...
        while not self.should_stop:
            self.train_loop(
                model, optimizer, train_loader, limit_batches=self.limit_train_batches, scheduler=scheduler
            )
    ...
    def train_loop(
        ...
        iterable = self.progbar_wrapper(
            train_loader, total=int(10**10//512000), desc=f"Epoch {self.current_epoch}"
        )
        ...
        for batch_idx, batch in enumerate(iterable):
            ...
            if self.global_step % self.checkpoint_frequency == 0:
                self.save({"model": model, "optim": optimizer})
    ...
    def training_step(self, model: L.LightningModule, batch: Any, batch_idx: int) -> torch.Tensor:
        ...
        outputs: Union[torch.Tensor, Mapping[str, Any]] = model(idx=x, targets=y)

        loss = outputs[1]
        # Enzo: I added this line to accumulate the loss and divide by the number of grad_accum_steps
